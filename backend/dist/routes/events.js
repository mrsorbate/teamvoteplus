...
router.get('/my-upcoming', (req, res) => {
    try {
        const now = new Date().toISOString();
        const events = init_1.default.prepare(`
      SELECT e.*,
    ...
    `).all(req.user.id, now, req.user.id);
        res.json((0, eventHelpers_1.attachTeamMetaToEvents)(events));
    }
    catch (error) {
        logger_1.logger.error('Get my upcoming events error:', error);
...
router.get('/my-all', (req, res) => {
    try {
      ...
      ORDER BY e.start_time ASC`;
        const events = init_1.default.prepare(myAllSql).all(req.user.id, now, req.user.id);
        res.json((0, eventHelpers_1.attachTeamMetaToEvents)(events));
    }
    catch (error) {
        logger_1.logger.error('Get my all events error:', error);
...
router.get('/', (req, res) => {
    ...
    `;
        ...
        const events = init_1.default.prepare(query).all(...params);
        res.json((0, eventHelpers_1.attachTeamMetaToEvents)(events));
    }
    catch (error) {
...
router.get('/:id', (req, res) => {
    try {
        ...
        const event = init_1.default.prepare(`
    ...
    `).get(eventId);
        ...
        const eventTeamIds = (0, eventHelpers_1.getEventTeamIds)(eventId);
        if (!(0, eventHelpers_1.isMemberOfAnyTeam)(req.user.id, eventTeamIds)) {
            return res.status(403).json({ error: 'Not a team member' });
        }
        const isTrainer = eventTeamIds.some((teamId) => {
            const m = init_1.default.prepare('SELECT role FROM team_members WHERE team_id = ? AND user_id = ?').get(teamId, req.user.id);
            return m?.role === 'trainer';
        });
        let responses = init_1.default.prepare(`
    ...
    `).all(eventId);
        ...
        const eventWithSeriesMeta = (0, eventHelpers_1.attachTeamMetaToEvent)({ ...event });
        if (event.series_id) {
            const seriesEvents = init_1.default.prepare('SELECT start_time FROM events WHERE series_id = ? ORDER BY start_time ASC').all(event.series_id);
            const repeatDaysSet = new Set();
            for (const se of seriesEvents) {
                const d = new Date(se.start_time);
                if (!Number.isNaN(d.getTime()))
                    repeatDaysSet.add(d.getDay());
            }
...
router.get('/:id/squad', (req, res) => {
    try {
        ...
        const event = init_1.default.prepare('SELECT id, team_id, type, created_by FROM events WHERE id = ?').get(eventId);
        if (!event)
            return res.status(404).json({ error: 'Event not found' });
        if (event.type !== 'match')
            return res.status(400).json({ error: 'Squad is only available for match events' });
        if (!(0, eventHelpers_1.isMemberOfAnyTeam)(req.user.id, (0, eventHelpers_1.getEventTeamIds)(eventId))) {
            return res.status(403).json({ error: 'Not a team member' });
        }
        const squadRow = init_1.default.prepare('SELECT event_id, squad_user_ids, lineup_slots, is_released, released_at, updated_at FROM event_match_squads WHERE event_id = ?').get(eventId);
        const payload = (0, eventHelpers_1.createMatchSquadResponse)(squadRow);
        const canManage = (0, eventHelpers_1.canManageEvent)(req.user.id, eventId, event.created_by);
        if (!canManage && payload.is_released !== 1) {
            return res.json({
                ...
                lineup_slot_order: eventHelpers_1.MATCH_LINEUP_SLOTS,
            });
        }
        return res.json({
            ...payload,
            event_id: eventId,
            squad_players: (0, eventHelpers_1.getMatchSquadPlayers)(event.team_id, payload.squad_user_ids),
        });
    }
    catch (error) {
        logger_1.logger.error('Get match squad error:', error);
        return res.status(500).json({ error: 'Failed to fetch match squad' });
    }
});
router.put('/:id/squad', (req, res) => {
    try {
        const eventId = parseInt(req.params.id, 10);
        if (!Number.isInteger(eventId) || eventId <= 0) {
            return res.status(400).json({ error: 'Invalid event id' });
        ...
        if (event.type !== 'match')
            return res.status(400).json({ error: 'Squad is only available for match events' });
        if (!(0, eventHelpers_1.canManageEvent)(req.user.id, eventId, event.created_by)) {
            return res.status(403).json({ error: 'Only trainers can edit match squad' });
        }
        const allowedMemberIds = new Set((0, eventHelpers_1.getMemberIdsForTeams)((0, eventHelpers_1.getEventTeamIds)(eventId)));
        const squadUserIds = (0, eventHelpers_1.normalizeSquadUserIds)(req.body?.squad_user_ids, allowedMemberIds);
        const lineupSlots = (0, eventHelpers_1.normalizeLineupSlots)(req.body?.lineup_slots, new Set(squadUserIds));
        init_1.default.prepare(`INSERT INTO event_match_squads (event_id, squad_user_ids, lineup_slots, is_released, released_at, updated_at)
       ...
       ON CONFLICT(event_id) DO UPDATE SET
         ...
         is_released = 0,
         released_at = NULL,
         updated_at = CURRENT_TIMESTAMP`).run(eventId, JSON.stringify(squadUserIds), JSON.stringify(lineupSlots));
        const updatedRow = init_1.default.prepare('SELECT event_id, squad_user_ids, lineup_slots, is_released, released_at, updated_at FROM event_match_squads WHERE event_id = ?').get(eventId);
        const payload = (0, eventHelpers_1.createMatchSquadResponse)(updatedRow);
        return res.json({
            ...payload,
            squad_players: (0, eventHelpers_1.getMatchSquadPlayers)(event.team_id, payload.squad_user_ids),
        });
        ...
        logger_1.logger.error('Update match squad error:', error);
        return res.status(500).json({ error: 'Failed to update match squad' });
    }
});
router.post('/:id/squad/release', (req, res) => {
    try {
        const eventId = parseInt(req.params.id, 10);
        if (!Number.isInteger(eventId) || eventId <= 0) {
            return res.status(400).json({ error: 'Invalid event id' });
        ...
        const existingSquad = init_1.default.prepare('SELECT event_id, squad_user_ids, lineup_slots, is_released, released_at, updated_at FROM event_match_squads WHERE event_id = ?').get(eventId);
        if (!existingSquad)
            return res.status(400).json({ error: 'Bitte zuerst einen Kader speichern' });
        const squadUserIds = (0, eventHelpers_1.parseStoredSquadUserIds)(existingSquad.squad_user_ids);
        if (squadUserIds.length === 0) {
            return res.status(400).json({ error: 'Bitte mindestens einen Spieler im Kader auswählen' });
        }
        init_1.default.prepare(`UPDATE event_match_squads
       SET is_released = 1, released_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP
       WHERE event_id = ?`).run(eventId);
        const releasedRow = init_1.default.prepare('SELECT event_id, squad_user_ids, lineup_slots, is_released, released_at, updated_at FROM event_match_squads WHERE event_id = ?').get(eventId);
        const payload = (0, eventHelpers_1.createMatchSquadResponse)(releasedRow);
        return res.json({
            ...payload,
            squad_players: (0, eventHelpers_1.getMatchSquadPlayers)(event.team_id, payload.squad_user_ids),
        });
        ...
        logger_1.logger.error('Release match squad error:', error);
...
router.post('/', async (req, res) => {
    try {
        const { team_id, team_ids, title, type, description, location, location_venue, location_street, location_zip_city, pitch_type, meeting_point, arrival_minutes, start_time, end_time, rsvp_deadline, duration_minutes, visibility_all = true, invite_all = true, invited_user_ids = [], repeat_type, repeat_until, repeat_days } = req.body;
        ...
        const targetTeamIds = (0, eventHelpers_1.normalizeTeamIds)(team_id, team_ids);
        if (!(0, eventHelpers_1.isTrainerForAllTeams)(req.user.id, targetTeamIds)) {
            return res.status(403).json({ error: 'Only trainers can create events' });
        }
        const targetTeamLabel = (0, eventHelpers_1.formatTeamLabel)((0, eventHelpers_1.getTeamNamesByIds)(targetTeamIds));
        const teamSettings = init_1.default.prepare(`SELECT default_response,
              ...
              home_venues
       FROM teams WHERE id = ?`).get(team_id);
        if (!(0, eventHelpers_1.hasMatchingPitchTypeInHomeVenues)(teamSettings?.home_venues, pitch_type)) {
        ...
        const defaultArrivalByType = {
            training: (0, eventHelpers_1.parseIntClamp)(teamSettings?.default_arrival_minutes_training, 0, 240),
            match: (0, eventHelpers_1.parseIntClamp)(teamSettings?.default_arrival_minutes_match, 0, 240),
            other: (0, eventHelpers_1.parseIntClamp)(teamSettings?.default_arrival_minutes_other, 0, 240),
        };
        const legacyArrival = (0, eventHelpers_1.parseIntClamp)(teamSettings?.default_arrival_minutes, 0, 240);
        ...
        const defaultDurationByType = {
            training: (0, eventHelpers_1.parseIntClamp)(teamSettings?.default_duration_minutes_training, 5, 480),
            match: (0, eventHelpers_1.parseIntClamp)(teamSettings?.default_duration_minutes_match, 5, 480),
            other: (0, eventHelpers_1.parseIntClamp)(teamSettings?.default_duration_minutes_other, 5, 480),
        };
        const legacyDuration = (0, eventHelpers_1.parseIntClamp)(teamSettings?.default_duration_minutes, 5, 480);
        const selectedDuration = defaultDurationByType[eventType] ?? legacyDuration;
        const resolvedDurationMinutes = (0, eventHelpers_1.parseIntClamp)(duration_minutes, 5, 480) ?? selectedDuration;
        ...
        const defaultRsvpByType = {
            training: (0, eventHelpers_1.parseIntClamp)(teamSettings?.default_rsvp_deadline_hours_training, 0, 168),
            match: (0, eventHelpers_1.parseIntClamp)(teamSettings?.default_rsvp_deadline_hours_match, 0, 168),
            other: (0, eventHelpers_1.parseIntClamp)(teamSettings?.default_rsvp_deadline_hours_other, 0, 168),
        };
        const legacyRsvp = (0, eventHelpers_1.parseIntClamp)(teamSettings?.default_rsvp_deadline_hours, 0, 168);
        const selectedRsvpHours = defaultRsvpByType[eventType] ?? legacyRsvp;
        const getDefaultRsvpDeadline = (eventStart) => {
            if (rsvp_deadline)
                return rsvp_deadline;
            if (selectedRsvpHours === null)
                return null;
        ...
        const memberIds = (0, eventHelpers_1.getMemberIdsForTeams)(targetTeamIds);
        ...
        if (isRecurring) {
            const seriesId = (0, crypto_1.randomBytes)(16).toString('hex');
            const startDate = new Date(start_time);
            const endDate = new Date(resolvedEndTime);
            const untilDate = (0, eventHelpers_1.parseRepeatUntilDate)(repeatUntilValue);
            const eventDates = (0, eventHelpers_1.generateRecurringDates)(startDate, endDate, repeat_type, untilDate, effectiveRepeatDays);
            if (eventDates.length === 0) {
                return res.status(400).json({ error: 'No valid dates generated for recurring event' });
            }
            const insertStmt = init_1.default.prepare('INSERT INTO events (team_id, title, type, description, location, location_venue, location_street, location_zip_city, pitch_type, meeting_point, arrival_minutes, start_time, end_time, rsvp_deadline, duration_minutes, visibility_all, invite_all, created_by, series_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
            ...
            for (const { start, end } of eventDates) {
                const result = insertStmt.run(team_id, title, type, description, resolvedLocation, location_venue || null, location_street || null, location_zip_city || null, pitch_type || null, resolvedMeetingPoint, resolvedArrivalMinutes, start.toISOString(), end.toISOString(), getDefaultRsvpDeadline(start.toISOString()), resolvedDurationMinutes, visibility_all ? 1 : 0, invite_all ? 1 : 0, req.user.id, seriesId);
                (0, eventHelpers_1.addEventTeamLinks)(Number(result.lastInsertRowid), targetTeamIds);
            ...
            if (invitedUserIds.length > 0) {
                ...
                await (0, pushNotifications_1.sendPushToUsers)(invitedUserIds, {
                    title: 'Neuer Termin',
            ...
            const insertStmt = init_1.default.prepare('INSERT INTO events (team_id, title, type, description, location, location_venue, location_street, location_zip_city, pitch_type, meeting_point, arrival_minutes, start_time, end_time, rsvp_deadline, duration_minutes, visibility_all, invite_all, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
            const result = insertStmt.run(team_id, title, type, description, resolvedLocation, location_venue || null, location_street || null, location_zip_city || null, pitch_type || null, resolvedMeetingPoint, resolvedArrivalMinutes, start_time, resolvedEndTime, getDefaultRsvpDeadline(start_time), resolvedDurationMinutes, visibility_all ? 1 : 0, invite_all ? 1 : 0, req.user.id);
            (0, eventHelpers_1.addEventTeamLinks)(Number(result.lastInsertRowid), targetTeamIds);
            const responseStmt = init_1.default.prepare('INSERT INTO event_responses (event_id, user_id, status) VALUES (?, ?, ?)');
            for (const userId of invitedUserIds)
                responseStmt.run(result.lastInsertRowid, userId, defaultResponseStatus);
...
router.put('/:id', (req, res) => {
    try {
        ...
        const { title, type, description, location, location_venue, location_street, location_zip_city, pitch_type, meeting_point, arrival_minutes, start_time, end_time, rsvp_deadline, duration_minutes, visibility_all, invite_all, invited_user_ids, repeat_until, repeat_days, } = req.body;
        if (!title || !type || !start_time || !end_time) {
            return res.status(400).json({ error: 'Missing required fields' });
        }
        const event = init_1.default.prepare('SELECT id, team_id, series_id, start_time, end_time, created_by FROM events WHERE id = ?').get(eventId);
        if (!event)
            return res.status(404).json({ error: 'Event not found' });
        const eventTeamIds = (0, eventHelpers_1.getEventTeamIds)(eventId);
        if (!(0, eventHelpers_1.canManageEvent)(req.user.id, eventId, event.created_by)) {
        ...
        const resolvedLocation = location_venue || location || null;
        const teamMemberIds = (0, eventHelpers_1.getMemberIdsForTeams)(eventTeamIds);
        ...
        const teamSettings = init_1.default.prepare('SELECT default_response, home_venues FROM teams WHERE id = ?').get(event.team_id);
        if (!(0, eventHelpers_1.hasMatchingPitchTypeInHomeVenues)(teamSettings?.home_venues, pitch_type)) {
            return res.status(400).json({ error: `Für die Platzart "${String(pitch_type || '').trim()}" ist kein Heimspiel-Platz hinterlegt` });
        }
        ...
        const defaultResponseStatus = validStatuses.has(String(teamSettings?.default_response || 'pending'))
            ...
            : 'pending';
        const updateStmt = init_1.default.prepare(`UPDATE events
       ...
       WHERE id = ?`);
        const arrivalVal = (v) => v === null || v === undefined || Number.isNaN(v) ? null : v;
        const durationVal = (v) => v === null || v === undefined || Number.isNaN(v) ? null : v;
        const visibilityVal = (v) => v === false || v === 0 ? 0 : 1;
        const syncInvitesForEvent = (targetEventId) => {
            const existing = init_1.default.prepare('SELECT user_id FROM event_responses WHERE event_id = ?').all(targetEventId);
            ...
            if (toRemove.length > 0) {
                ...
                init_1.default.prepare(`DELETE FROM event_responses WHERE event_id = ? AND user_id IN (${ph})`).run(targetEventId, ...toRemove);
            }
            if (toAdd.length > 0) {
                const stmt = init_1.default.prepare('INSERT INTO event_responses (event_id, user_id, status) VALUES (?, ?, ?)');
                for (const uid of toAdd)
                    stmt.run(targetEventId, uid, defaultResponseStatus);
            }
        };
        if (shouldReshapeSeries) {
            ...
            const seriesEvents = init_1.default.prepare('SELECT id, start_time FROM events WHERE series_id = ? ORDER BY start_time ASC').all(event.series_id);
            ...
            generationStart.setUTCHours(targetStartDate.getUTCHours(), targetStartDate.getUTCMinutes(), targetStartDate.getUTCSeconds(), targetStartDate.getUTCMilliseconds());
            const generationEnd = new Date(generationStart.getTime() + targetDurationMs);
            const generatedDates = (0, eventHelpers_1.generateRecurringDates)(generationStart, generationEnd, 'custom', untilDate, normalizedRepeatDays);
            if (generatedDates.length === 0)
                return res.status(400).json({ error: 'Keine gültigen Termine für die Serie erzeugt' });
            const remainingExisting = [...seriesEvents];
            const createStmt = init_1.default.prepare('INSERT INTO events (team_id, title, type, description, location, location_venue, location_street, location_zip_city, pitch_type, meeting_point, arrival_minutes, start_time, end_time, rsvp_deadline, duration_minutes, visibility_all, invite_all, created_by, series_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
            init_1.default.transaction(() => {
                for (const gd of generatedDates) {
                    ...
                    if (existingIdx >= 0) {
                        ...
                        updateStmt.run(title, type, description || null, resolvedLocation, location_venue || null, location_street || null, location_zip_city || null, pitch_type || null, meeting_point || null, arrivalVal(arrival_minutes), gd.start.toISOString(), gd.end.toISOString(), nextRsvpDeadline, durationVal(duration_minutes), visibilityVal(visibility_all), resolvedInviteAll ? 1 : 0, event.series_id, existingEvent.id);
                        syncInvitesForEvent(existingEvent.id);
                    }
                    else {
                        const inserted = createStmt.run(event.team_id, title, type, description || null, resolvedLocation, location_venue || null, location_street || null, location_zip_city || null, pitch_type || null, meeting_point || null, arrivalVal(arrival_minutes), gd.start.toISOString(), gd.end.toISOString(), nextRsvpDeadline, durationVal(duration_minutes), visibilityVal(visibility_all), resolvedInviteAll ? 1 : 0, event.created_by, event.series_id);
                        (0, eventHelpers_1.addEventTeamLinks)(Number(inserted.lastInsertRowid), eventTeamIds);
                ...
                if (remainingExisting.length > 0) {
                    ...
                    if (idsToDelete.length > 0) {
                        ...
                        init_1.default.prepare(`DELETE FROM event_responses WHERE event_id IN (${ph})`).run(...idsToDelete);
                        init_1.default.prepare(`DELETE FROM events WHERE id IN (${ph})`).run(...idsToDelete);
                    }
                }
            })();
        ...
        const eventsToUpdate = updateSeries && event.series_id
            ? init_1.default.prepare('SELECT id, start_time FROM events WHERE series_id = ?').all(event.series_id)
...
router.post('/:id/response', (req, res) => {
    try {
        const eventId = parseInt(req.params.id, 10);
        const schemaResult = validation_1.updateEventResponseSchema.safeParse(req.body);
        ...
        const event = init_1.default.prepare('SELECT id, team_id, rsvp_deadline FROM events WHERE id = ?').get(eventId);
        if (!event)
            return res.status(404).json({ error: 'Event not found' });
        ...
        const membership = eventTeamIds
            .map((teamId) => init_1.default.prepare('SELECT role FROM team_members WHERE team_id = ? AND user_id = ?').get(teamId, req.user.id))
            .find(Boolean);
        if (!membership)
            return res.status(403).json({ error: 'Not a team member' });
    ...
    `).run(eventId, req.user.id, status, normalizedComment || null, status, normalizedComment || null);
        ...
        logger_1.logger.error('Update response error:', error);
        res.status(500).json({ error: 'Failed to update response' });
    }
});
// Update event response for a specific user (trainer only)
router.post('/:id/response/:userId', (req, res) => {
    try {
        const eventId = parseInt(req.params.id, 10);
        ...
        const { status, comment } = schemaResult.data;
        const normalizedComment = comment?.trim() ?? '';
        const event = init_1.default.prepare('SELECT id, team_id, rsvp_deadline, created_by FROM events WHERE id = ?').get(eventId);
        if (!event)
            return res.status(404).json({ error: 'Event not found' });
        if (status === 'tentative' && event.rsvp_deadline) {
            const deadlineDate = new Date(event.rsvp_deadline);
        ...
        if (!(0, eventHelpers_1.isMemberOfAnyTeam)(userId, eventTeamIds)) {
    ...
    `).run(eventId, userId, status, normalizedComment || null, status, normalizedComment || null);
        ...
        logger_1.logger.error('Update response for user error:', error);
        res.status(500).json({ error: 'Failed to update response' });
    }
});
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const event = init_1.default.prepare('SELECT id, team_id, series_id, title, start_time, created_by FROM events WHERE id = ?').get(eventId);
        if (!event)
            return res.status(404).json({ error: 'Event not found' });
        if (!(0, eventHelpers_1.canManageEvent)(req.user.id, eventId, event.created_by)) {
            return res.status(403).json({ error: 'Only trainers can delete events' });
        }
        const upsertDeleted = init_1.default.prepare(`INSERT INTO deleted_events (event_id, team_id, title, start_time, end_time, deleted_at)
       ...
       ON CONFLICT(event_id) DO UPDATE SET
        ...
        if (deleteSeries && event.series_id) {
            const eventsInSeries = init_1.default.prepare('SELECT id, team_id, title, start_time, end_time FROM events WHERE series_id = ?').all(event.series_id);
            const seriesTeamLabel = (0, eventHelpers_1.formatTeamLabel)((0, eventHelpers_1.getEventTeams)(eventId).map((t) => t.name));
            const teamIds = [...new Set(eventsInSeries.map((e) => e.team_id))];
            const allTeamMembers = (0, eventHelpers_1.getMemberIdsForTeams)(Array.from(teamIds));
            const notifyUserIds = allTeamMembers.filter((uid) => uid !== req.user.id);
            const result = init_1.default.transaction(() => {
                for (const er of eventsInSeries) {
                    upsertDeleted.run(er.id, er.team_id, er.title || null, er.start_time || null, er.end_time || null);
                }
                return init_1.default.prepare('DELETE FROM events WHERE series_id = ?').run(event.series_id);
            })();
            if (notifyUserIds.length > 0) {
                const noteSuffix = deleteNote ? ` Hinweis: ${deleteNote}` : '';
                await (0, pushNotifications_1.sendPushToUsers)(notifyUserIds, {
            ...
            const eventToDelete = init_1.default.prepare('SELECT id, team_id, title, start_time, end_time FROM events WHERE id = ?').get(eventId);
            if (!eventToDelete)
                return res.status(404).json({ error: 'Event not found' });
            const singleTeamLabel = (0, eventHelpers_1.formatTeamLabel)((0, eventHelpers_1.getEventTeams)(eventId).map((t) => t.name));
            const teamIds = (0, eventHelpers_1.getEventTeamIds)(eventId);
            const allTeamMembers = (0, eventHelpers_1.getMemberIdsForTeams)(teamIds);
            const notifyUserIds = allTeamMembers.filter((uid) => uid !== req.user.id);
            init_1.default.transaction(() => {
                upsertDeleted.run(eventToDelete.id, eventToDelete.team_id, eventToDelete.title || null, eventToDelete.start_time || null, eventToDelete.end_time || null);
                init_1.default.prepare('DELETE FROM events WHERE id = ?').run(eventId);
            })();
            if (notifyUserIds.length > 0) {
                const noteSuffix = deleteNote ? ` Hinweis: ${deleteNote}` : '';
                await (0, pushNotifications_1.sendPushToUsers)(notifyUserIds, {

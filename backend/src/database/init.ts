...
try {
  ...
  const postTable = db.prepare("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'team_posts'").get() as { sql?: string } | undefined;
  if (postTable?.sql && !postTable.sql.includes("'event'")) {
    ...
    `);
    logger.info('Recreated team_posts table with extended feed post types');
  }
